    ...
    try:
        ...
        for tr in soup.select_one('.bpan table').select('tr'):
            for td in tr.select('td'):
                if td.text == 'Album' or td.text == 'Category':
        ...
        data['acf_fields']['download_links'] = ''.join([urljoin(str(url), '/')[:-1], soup.select_one('.dbutton').find('a')['href']]) 
        return data
